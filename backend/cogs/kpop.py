...
class Kpop(commands.Cog):
    ...
    async def check_playlist_updates(self):
        ...
        result = await self.get_newest_songs_from_playlist("2Mq9TtE1Hv3c20UvuX3UwB", "music/kpop_music.json")
        if result:
            for item in result:
        ...
        result = await self.get_newest_songs_from_playlist("37i9dQZF1DWUa8ZRTfalHk", "music/pop_music.json")
        if result:
            for item in result:
                track_name, artist, spotify_link, image_url, artist_id= item  # Include image_url
